...
int createSharedObject(int32 ownerID, char* shareName, uint32 size, uint8 isWritable, void* virtual_address)
{
	...
	uint32 vaStart = ROUNDDOWN((uint32)virtual_address,PAGE_SIZE);
	uint32 vaEnd = ROUNDUP((uint32)virtual_address +size, PAGE_SIZE);
	uint32 numOf_Pages = (vaEnd-vaStart)/PAGE_SIZE;
	if(get_share_object_ID(ownerID,shareName)==E_SHARED_MEM_NOT_EXISTS)
	{
		int shared_id= allocate_share_object(&allocatedobject);
		if(shared_id == MAX_SHARES)
		{
			return E_NO_SHARE;
		}
		else
		...
		{
			uint32 newAdd = vaStart + (i*PAGE_SIZE);
			uint32 * ptrPAGETABLE = NULL;
			struct FrameInfo* ptrFrame= get_frame_info(myenv->env_page_directory,newAdd,&ptrPAGETABLE);
			if(ptrFrame != NULL)
			{
				return -1;
			}
			 int ret=allocate_frame(&ptrFrame);
			 ret=map_frame(myenv->env_page_directory,ptrFrame,newAdd,PERM_WRITEABLE|PERM_USER);
			 ptrFrame->va = newAdd;
			add_frame_to_storage(shares[shared_id].framesStorage,ptrFrame,i);
		}
		 shares[shared_id].ownerID=ownerID;
		 strcpy(shares[shared_id].name,shareName);
		 shares[shared_id].size=size;
		 shares[shared_id].references=1;
		 shares[shared_id].isWritable=isWritable;
		 return shared_id;
		}
		}
		else
...
int getSharedObject(int32 ownerID, char* shareName, void* virtual_address)
{
	...
	int shareGet = get_share_object_ID(ownerID,shareName);
	uint32 vaStart = ROUNDDOWN((uint32)virtual_address,PAGE_SIZE);
	uint32 vaEnd = ROUNDUP((uint32)virtual_address, PAGE_SIZE);
	uint32 numOfLoops = (shares[shareGet].size)/PAGE_SIZE;
	for(int i = 0 ; i< numOfLoops ;i++)
	{
		uint32 newAdd = vaStart + (i*PAGE_SIZE);
		struct FrameInfo* ptrFrame = get_frame_from_storage(shares[shareGet].framesStorage,i);
		ptrFrame->va = (uint32)virtual_address;
		if(shares[shareGet].isWritable == 1){
			map_frame(myenv->env_page_directory,ptrFrame,newAdd,PERM_WRITEABLE|PERM_USER);
		}
		else
		{
			map_frame(myenv->env_page_directory,ptrFrame,newAdd,PERM_USER);
		}
		shares[shareGet].references++;
		return shareGet;
	}
	return E_SHARED_MEM_NOT_EXISTS;
